...
elasticsearchSqlApp.controller('MainController', function ($scope, $http, $sce,$compile) {
  ...
  scroll_url = "api/SqlApi/scroll?scroll=1m&scroll_id=";
	$scope.url = getUrl();
...
function updateWithScrollIfNeeded (query) {
  ...
  return query;
    ...
    //Edite here
	//$http.post($scope.url + "SqlApi/execute", query)
    $http.post($scope.url + "api/SqlApi/execute?sql="+encodeURIComponent(query))
		.success(function(data, status, headers, config) {
          var handler = ResultHandlerFactory.create(data,$scope.config.isFlat,$scope.config.showScore,$scope.config.showType);
          updateDescription(handler);
          if(handler.isScroll){
    ...
    $http.post($scope.url + "api/SqlApi/explain?sql="+encodeURIComponent(query))
		.success(function(data, status, headers, config) {
					 $scope.resultExplan = true;
