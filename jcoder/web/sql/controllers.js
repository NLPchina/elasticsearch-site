...
elasticsearchSqlApp.controller('MainController', function ($scope, $http, $sce, $compile) {
    ...
    $scope.nextSearch = function () {
        ...
        $http.get("/api/SqlApi/scroll?scrollId=" + $scope.scrollId)
    ...
    function updateWithScrollIfNeeded(query) {
        if (!$scope.config.alwaysScroll) return query;
        if (query.indexOf("USE_SCROLL") != -1) return query;
        var scrollHint = "/*! USE_SCROLL(" + $scope.config.scrollSize + "," + 120000 + ") */";
        if (query.indexOf("select") != -1) return query.replace("select", "select " + scrollHint);
        if (query.indexOf("SELECT") != -1) return query.replace("SELECT", "select " + scrollHint);
    ...
    $scope.search = function () {
        ...
        $http.post("/api/SqlApi/execute", $.param({sql: query}), {headers: {'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;'}})
            .success(function (data, status, headers, config) {
                var handler = ResultHandlerFactory.create(data, $scope.config.isFlat, $scope.config.showScore, $scope.config.showType, $scope.config.showId);
                updateDescription(handler);
                if (handler.isScroll) {
                    ...
                    if ($scope.config.isAutoSave) {
                        ...
                        $scope.gotNext = true;
                        var scrollBudy = handler.getBody();
                        //not using scan type
    ...
    $scope.explain = function () {
        ...
        var query = window.editor.getValue();
        $http.post("/api/SqlApi/explain", $.param({sql: query}), {headers: {'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8;'}})
            .success(function (data, status, headers, config) {
                $scope.resultExplan = true;
    ...
    function callScrollAndFillBodyTillEnd(scrollId, head, body, firstTime, needToExport, updatePresenter) {
        $http.get("/api/SqlApi/scroll?scrollId=" + scrollId)
            .success(function (data, status, headers, config) {
                var handler = ResultHandlerFactory.create(data, $scope.isFlat);
                updateDescription(handler);
                var recieved = handler.getBody();
                if (body.length > 0) {
